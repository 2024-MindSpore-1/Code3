...
def train(args):
    ...
    if "replknet" in args.model:
        if args.ckpt_path != '' and os.path.exists(args.ckpt_path):
            from mindspore import load_checkpoint, load_param_into_net
            param_not_load, ckpt_not_load = load_param_into_net(network, load_checkpoint(args.ckpt_path))
            logger.info(f"load_checkpoint into {args.model} from {args.ckpt_path}")
            args.ckpt_path = ''
            logger.info(f"================== param_not_load ==================")
            logger.info(param_not_load)
            logger.info(f"================== ckpt_not_load ==================")
            logger.info(ckpt_not_load)
...
if __name__ == "__main__":
    ...
    if args.enable_modelarts:
        from mox_adapter import sync_data

        src_url = eval(args.multi_data_url.replace('true', "True"))[0]['dataset_url']
        sync_data(from_path=src_url, to_path="/cache/imagenet.zip", threads=256, unzip=True)
        args.data_dir = "/cache/imagenet"
