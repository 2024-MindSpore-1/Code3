...
class NetWithLoss(nn.Cell):
    ...
    def construct(self, data1, data2, data3, label):
        ...
        loss1 = self.criterion(predict1, label)
        ...
        '''
        length = len(data1)
        loss = Tensor(0.0, dtype=mstype.float32)
        
        for i in range(length):


            x = self.model(data1[i], data2[i], data3[i])
            loss = loss + self.criterion(x, label[i]) / length
                
 
                
        #loss = x[0] - label
        '''
        x = self.model(data1, data2, data3)
