...
class Inference(nn.Cell):
    ...
    def construct(self, images, labels, is_train=False):
        ...
        mae = self.cal_mae(counting_preds, gen_counting_label(labels, self.out_channel, True)).asnumpy()
        mse = math.sqrt(self.cal_mse(counting_preds, gen_counting_label(labels, self.out_channel, True)).asnumpy())
...
class AttDecoder(nn.Cell):
    def __init__(self, params):
        ...
        self.encoder_feature_conv = nn.Conv2d(self.out_channel, self.attention_dim, kernel_size=1, has_bias=True)
        ...
        self.counting_context_weight = nn.Dense(self.word_num, self.hidden_size)
        self.word_convert = nn.Dense(self.hidden_size, self.word_num)

        if params['dropout']:
            self.dropout = nn.Dropout(params['dropout_ratio'])

    def construct(self, cnn_features, counting_preds, is_train=False):
        batch_size, _, height, width = cnn_features.shape
        ones = ops.Ones()
        image_mask = ones((batch_size, 1, height, width),ms.float32)
        
        cnn_features_trans = self.encoder_feature_conv(cnn_features)
        position_embedding = PositionEmbeddingSine(256, normalize=True)
        image_mask_numpy = image_mask.asnumpy()
        pos = position_embedding(cnn_features_trans, ms.Tensor(image_mask_numpy[:,0,:,:],ms.float32))
