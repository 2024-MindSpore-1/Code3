...
class HMERDataset:
    ...
    def __getitem__(self, idx):
        ...
        image = ms.Tensor([(255-image)/255],dtype=ms.float32)
        labels.append('eos')
        words = self.words.encode(labels)
        words = ms.Tensor(words,dtype=ms.int64)
        return image, words
...
def get_crohme_dataset(params):
    ...
    train_loader = ds.GeneratorDataset(train_dataset, ['data','label'],shuffle=False,num_parallel_workers=params['workers'])
    eval_loader = ds.GeneratorDataset(eval_dataset, ['data','label'], shuffle=False,num_parallel_workers=params['workers'])
    
    print(f'train dataset: {len(train_dataset)} train steps: {train_loader.get_dataset_size()} '
          f'eval dataset: {len(eval_dataset)} eval steps: {eval_loader.get_dataset_size()} ')
    return train_loader, eval_loader


def collate_fn(batch_images,labels,info):
    max_width, max_height, max_length = 0, 0, 0
    batch, channel = len(batch_images), batch_images[0].shape[0]
    proper_items = []
    for item in zip(batch_images,labels):
    ...
    labels, labels_masks = zeros((len(proper_items), max_length),ms.int32), zeros((len(proper_items), max_length),ms.float32)

    for i in range(len(proper_items)):
        _, h, w = proper_items[i][0].shape
        
        images_numpy = images.asnumpy()
        images_numpy[i][:, :h, :w] = proper_items[i][0]
        ...
        labels_numpy[i][:l] = proper_items[i][1]
        labels = ms.Tensor(labels_numpy)
        
        labels_masks_numpy = labels_masks.asnumpy()
        labels_masks_numpy[i][:l] = 1
        labels_masks = ms.Tensor(labels_masks_numpy)
