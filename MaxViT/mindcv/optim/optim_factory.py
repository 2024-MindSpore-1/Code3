...
def init_group_params_replknet(params, weight_decay, layer_decay):
    num_layers = 12
    output_params = []
    scale = []
    for param in params:
        if param.name.startswith("stem"):
            i = 0
        elif param.name.startswith("blocks"):
            list_index = [0, 1] + list(range(3, 5)) + list(range(6, 24)) + list(range(25, 27))
            index = eval(param.name.split('.')[1])
            if "conv" in param.name:
                ...
                i = list_index.index(index) // 2 + 1
        else:
            i = 13
        scale.append(layer_decay ** (num_layers + 1 - i))
        if "beta" not in param.name and "gamma" not in param.name and "bias" not in param.name:
            output_params.append({"params": [param, ], "weight_decay": 0.})
        else:
            output_params.append({"params": [param, ], "weight_decay": weight_decay})

    # output_params.append({"order_params": params})
    return output_params, scale
...
def create_optimizer(
    ...
    if weight_decay and filter_bias_and_bn:
        if "replknet" in model_name:
            params, layer_decay_scale = init_group_params_replknet(params, weight_decay, layer_decay)
        else:
    ...
    # adaptive
        ...
        optimizer = AdamW(
            ...
            weight_decay=weight_decay,
            loss_scale=loss_scale,
            layer_decay_scale=layer_decay_scale,
            **opt_args,
        )
    elif opt == "lion":
        optimizer = Lion(
