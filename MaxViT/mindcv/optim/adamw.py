...
def _update_run_op(
    ...
    if optim_filter:
        ...
        update_with_lr = ops.mul(lr * layer_decay, update)
...
class AdamW(Optimizer):
    ...
    def __init__(
        ...
        self.clip = clip
        if isinstance(layer_decay_scale, tuple) or isinstance(layer_decay_scale, list):
            self.layer_decay_scale = tuple(Tensor(decay, ms.float32) for decay in layer_decay_scale)
            assert len(self.layer_decay_scale) == len(self.weight_decay)
        else:
            self.layer_decay_scale = tuple(Tensor(layer_decay_scale, ms.float32) for _ in range(len(self.weight_decay)))
