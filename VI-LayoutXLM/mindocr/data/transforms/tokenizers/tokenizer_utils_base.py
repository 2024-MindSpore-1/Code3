...
class SpecialTokensMixin:
    ...
    def add_special_tokens(self, special_tokens_dict: Dict[str, Union[str, AddedToken]]) -> int:
        ...
        for key, value in special_tokens_dict.items():
            ...
            if self.verbose:
                _logger.info(f"Assigning {value} to the {key} key of the tokenizer")
            setattr(self, key, value)
    ...
    def bos_token(self) -> str:
        ...
        if self._bos_token is None and self.verbose:
            _logger.error("Using bos_token, but it is not set yet.")
            return None
    ...
    def eos_token(self) -> str:
        ...
        if self._eos_token is None and self.verbose:
            _logger.error("Using eos_token, but it is not set yet.")
            return None
    ...
    def unk_token(self) -> str:
        ...
        if self._unk_token is None and self.verbose:
            _logger.error("Using unk_token, but it is not set yet.")
            return None
    ...
    def sep_token(self) -> str:
        ...
        if self._sep_token is None and self.verbose:
            _logger.error("Using sep_token, but it is not set yet.")
            return None
    ...
    def pad_token(self) -> str:
        ...
        if self._pad_token is None and self.verbose:
            _logger.error("Using pad_token, but it is not set yet.")
            return None
    ...
    def cls_token(self) -> str:
        ...
        if self._cls_token is None and self.verbose:
            _logger.error("Using cls_token, but it is not set yet.")
            return None
    ...
    def mask_token(self) -> str:
        ...
        if self._mask_token is None and self.verbose:
            _logger.error("Using mask_token, but it is not set yet.")
            return None
    ...
    def additional_special_tokens(self) -> List[str]:
        ...
        if self._additional_special_tokens is None and self.verbose:
            _logger.error("Using additional_special_tokens, but it is not set yet.")
            return None
...
class PretrainedTokenizerBase(SpecialTokensMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, from_hf_hub=False, **kwargs):
        ...
        # From HF Hub
        if from_hf_hub:
            raise NotImplementedError("Loading from HF hub is not implemented.")
        # From built-in pretrained models
        elif pretrained_model_name_or_path in cls.pretrained_init_configuration:
            raise NotImplementedError("Loading from built-in pretrained models is not implemented.")
        # From local dir path
            ...
            raise NotImplementedError("Loading from community-contributed pretrained models is not implemented.")

        default_root = pretrained_model_name_or_path
        ...
        for file_id, file_path in vocab_files.items():
            ...
            if from_hf_hub:
                ...
                if os.path.exists(path):
                    _logger.info("Already cached %s" % path)
                    resolved_vocab_files[file_id] = path
                else:
                    raise NotImplementedError("Downloading pretrained model is not implemented.")
    ...
    def save_pretrained(self, save_directory, filename_prefix: Optional[str] = None, **kwargs):
        ...
        _logger.info(f"tokenizer config file saved in {tokenizer_config_file}")

        # Sanitize AddedTokens in special_tokens_map
        write_dict = convert_added_tokens(self.special_tokens_map_extended, add_type_field=False)
        with open(special_tokens_map_file, "w", encoding="utf-8") as f:
            f.write(json.dumps(write_dict, ensure_ascii=False))
        _logger.info(f"Special tokens file saved in {special_tokens_map_file}")
    ...
    def _save_pretrained(
        ...
        if added_vocab:
            with open(added_tokens_file, "w", encoding="utf-8") as f:
                ...
                f.write(out_str)
                _logger.info(f"added tokens file saved in {added_tokens_file}")
        ...
        **kwargs
        ...
        Args:
            ...
            return_tensors (str or [TensorType], optional):
                ...
                - `'ms'`: Return MindSpore `Tensor` objects.
    ...
    def pad(
        ...
        if not isinstance(first_element, (int, list, tuple)):
            if isinstance(first_element, Tensor):
