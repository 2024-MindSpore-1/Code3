...
from ..utils.vd_resnet_cells import ConvNormLayer, DeConvNormLayer
...
class PGFPN(nn.Cell):
    def __init__(self, in_channels: List[int], out_channels: int = 128, pytorch_like: bool = False, **kwargs):
        super().__init__()
        assert len(in_channels) == 7
        # self.out_channels = out_channels
        self.out_channels = 128

        in_channels  = [ 3, 64, 256]
        out_channels = [32, 64, 128]

        # 缩小 + 升维
        self.conv_bn_layer_1 = ConvNormLayer(
            in_channels=in_channels[0],
            out_channels=out_channels[0],
            kernel_size=3)
        self.conv_bn_layer_2 = ConvNormLayer(
            in_channels=in_channels[1],
            out_channels=out_channels[1],
            kernel_size=3)
        self.conv_bn_layer_3 = ConvNormLayer(
            in_channels=in_channels[2],
            out_channels=out_channels[2],
            kernel_size=3)

        # 缩小 + 升维
        self.conv_bn_layer_4 = ConvNormLayer(
            in_channels=out_channels[0],
            out_channels=out_channels[1],
            kernel_size=3,
            stride=2,
            padding=1 if pytorch_like else -1)  # NOTE 特殊
        self.conv_bn_layer_5 = ConvNormLayer(
            in_channels=out_channels[1],
            out_channels=out_channels[1],
            kernel_size=3,
            act=True)
        self.conv_bn_layer_6 = ConvNormLayer(
            in_channels=out_channels[1],
            out_channels=out_channels[2],
            kernel_size=3,
            stride=2,
            padding=1 if pytorch_like else -1)  # NOTE 特殊
        self.conv_bn_layer_7 = ConvNormLayer(
            in_channels=out_channels[2],
            out_channels=out_channels[2],
            kernel_size=3,
            act=True)

        self.conv_bn_layer_8 = ConvNormLayer(
            in_channels=out_channels[2],
            out_channels=out_channels[2],
            kernel_size=1)

        in_channels  = [2048, 2048, 1024, 512, 256]
        out_channels = [ 256,  256,  192, 192, 128]

        # 降维
        self.conv_h0 = ConvNormLayer(
            in_channels=in_channels[0],
            out_channels=out_channels[0],
            kernel_size=1)
        self.conv_h1 = ConvNormLayer(
            in_channels=in_channels[1],
            out_channels=out_channels[1],
            kernel_size=1)
        self.conv_h2 = ConvNormLayer(
            in_channels=in_channels[2],
            out_channels=out_channels[2],
            kernel_size=1)
        self.conv_h3 = ConvNormLayer(
            in_channels=in_channels[3],
            out_channels=out_channels[3],
            kernel_size=1)
        self.conv_h4 = ConvNormLayer(
            in_channels=in_channels[4],
            out_channels=out_channels[4],
            kernel_size=1)

        # 衔接尺寸和维度
        self.dconv0 = DeConvNormLayer(
            in_channels=out_channels[0],
            out_channels=out_channels[1])
        self.dconv1 = DeConvNormLayer(
            in_channels=out_channels[1],
            out_channels=out_channels[2])
        self.dconv2 = DeConvNormLayer(
            in_channels=out_channels[2],
            out_channels=out_channels[3])
        self.dconv3 = DeConvNormLayer(
            in_channels=out_channels[3],
            out_channels=out_channels[4])

        # 缩小
        self.conv_g1 = ConvNormLayer(
            in_channels=out_channels[1],
            out_channels=out_channels[1],
            kernel_size=3,
            act=True)
        self.conv_g2 = ConvNormLayer(
            in_channels=out_channels[2],
            out_channels=out_channels[2],
            kernel_size=3,
            act=True)
        self.conv_g3 = ConvNormLayer(
            in_channels=out_channels[3],
            out_channels=out_channels[3],
            kernel_size=3,
            act=True)
        self.conv_g4 = ConvNormLayer(
            in_channels=out_channels[4],
            out_channels=out_channels[4],
            kernel_size=3,
            act=True)

        self.convf = ConvNormLayer(
            in_channels=out_channels[4],
            out_channels=out_channels[4],
            kernel_size=1)
        self.act = nn.ReLU()
    ...
    def construct(self, features: List[Tensor]) -> Tensor:
        c0, c1, c2, c3, c4, c5, c6 = features

        # FPN_Down_Fusion
        h = [self.conv_bn_layer_1(c0),
             self.conv_bn_layer_2(c1),
             self.conv_bn_layer_3(c2)]

        g = self.conv_bn_layer_4(h[0]) + h[1]
        g = self.conv_bn_layer_5(self.act(g))

        g = self.conv_bn_layer_6(g) + h[2]
        g = self.conv_bn_layer_7(self.act(g))

        f_down = self.conv_bn_layer_8(g)
        ...
        g = self.dconv0(h[0]) + h[1]
        g = self.conv_g1(self.act(g))

        g = self.dconv1(g) + h[2]
        g = self.conv_g2(self.act(g))

        g = self.dconv2(g) + h[3]
        g = self.conv_g3(self.act(g))

        g = self.dconv3(g) + h[4]
        g = self.conv_g4(self.act(g))

        f_up = self.convf(g)

        f_common = f_down + f_up
        f_common = self.act(f_common)
        return f_common
