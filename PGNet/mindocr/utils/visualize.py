...
def draw_texts_with_polygons(
    ...
    for i, text in enumerate(texts):
        # draw text on the most left-top point
        box = bboxes[i]
        draw_point = box[np.argmin(np.array(box).sum(axis=1))]

        # TODO: use other lib which can set font size dynamically after font loading
        font = ImageFont.load_default() if not font_path else ImageFont.truetype(font_path, font_size, encoding="utf-8")
...
def visualize(
    ...
    if texts is not None:
        if draw_texts_on_blank_page:
            ...
            font_size = max(int(18 * rgb_img.shape[0] / 700), 30)
            text_color = (0, 0, 0)
        else:
            bg = det_vis
            hide_boxes = True
        ...
        if is_polygon:
            text_vis = draw_texts_with_polygons(
                bg,
                boxes,
                texts,
