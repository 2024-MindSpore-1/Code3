...
__all__ = ["EvalSaveCallback", "ProfilerCallback"]
_logger = logging.getLogger(__name__)


class ProfilerCallback(Callback):
    def __init__(self, output_path: str, use_epoch: bool, start_step: int = 1, stop_step: int = 10):
        super().__init__()
        self.start_step = start_step
        self.stop_step = stop_step
        self.use_epoch = use_epoch
        self.profiler = ms.Profiler(output_path=output_path, start_profile=False)

    def step_begin(self, run_context):
        cb_params = run_context.original_args()
        if self.use_epoch:
            step_num = cb_params.cur_epoch_num
        else:
...
class EvalSaveCallback(Callback):
    ...
    def on_train_epoch_end(self, run_context):
        ...
        if data_sink_mode:
            ...
            opt = cb_params.train_network.network.optimizer
        else:
        ...
        cur_lr = float(cur_lr) if not isinstance(cur_lr, (Tuple, List)) else [float(lr) for lr in cur_lr]
        if isinstance(cur_lr, List):
            cur_lr = set(cur_lr).pop()  # if group lr, get the first lr
            lr_str = f"lr_0: {cur_lr:.6f}, "
        else:
            lr_str = f"lr: {cur_lr:.6f}, "
