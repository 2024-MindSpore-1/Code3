...
def main(cfg):
    ...
    if cfg.system.distribute:
        ...
        ms.set_auto_parallel_context(
            ...
            gradients_mean=True,
            # parameter_broadcast=True,  # FIXME 程序卡死
        )
        set_logger(
            name="mindocr", 
            output_dir=cfg.train.ckpt_save_dir or "./", 
    ...
    if "name" in cfg.model:
        ...
        model_name = f"{cfg.model.backbone.name}-{cfg.model.neck.name}-{cfg.model.head.name}"
    if cfg.model.type in ["det", "e2e"]:
        det_spec = f"Allow rescaling polygons for Det/E2E postprocess: {allow_postprocess_rescale}"
        rec_spec = ""
    else: 
        det_spec = ""
        rec_spec = (f"Character dict path: {cfg.common.character_dict_path}\n"
                    f"Use space char: {cfg.common.use_space_char}\n"
