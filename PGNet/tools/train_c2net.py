...
import logging
import os
os.environ['MS_DEV_DISABLE_PREBUILD'] = 'True'
import shutil
import sys
...
from tools.arg_parser_c2net import parse_args_and_config
...
def main(cfg):
    ...
    if cfg.system.distribute:
        ...
        ms.set_auto_parallel_context(
            ...
            gradients_mean=True,
            # parameter_broadcast=True,  # 有大问题，千万不能开
        )
...
if __name__ == "__main__":
    ...
    if args.enable_modelarts:
        ...
        from tools.modelarts_adapter.modelarts import get_device_id, update_config_value_by_key
        from tools.modelarts_adapter.c2net import multidataset_to_env, pretrain_to_env
        ...
        if args.multi_data_url:
            multidataset_to_env(args.multi_data_url, dataset_root)
        else:
            multidataset_to_env(args.data_url, dataset_root)
        if get_device_id() == 0:
            print(
        ...
        if args.pretrain_url:
            ...
            pretrain_file = pretrain_to_env(args.pretrain_url, ckpt_root)
            config.model.pretrained = pretrain_file
    ...
    if args.enable_modelarts:
        ...
        if get_device_id() == 0:
            if config.train.ckpt_save_dir != args.train_url:
