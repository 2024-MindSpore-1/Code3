...
def get_args():
    ...
    parser.add_argument(
        "--e2e_model_path", type=str, required=False, help="End2End Detection and Recognition model file path."
    )
    parser.add_argument(
        "--e2e_model_name_or_config",
        type=str,
        required=False,
        help="End2End Detection and Recognition model name or config file path.",
    )
...
def update_task_info(args):
    ...
    if e2e:
        setattr(args, "task_type", TaskType.E2E)
    elif task_order in task_map:
    ...
    if args.e2e_model_name_or_config:
        setattr(args, "e2e_config_path", get_config_by_name_for_model(args.e2e_model_name_or_config))
...
def check_and_update_args(args):
    ...
    if args.e2e_model_path and not args.e2e_model_name_or_config:
        raise ValueError("e2e_model_name_or_config can't be emtpy when set e2e_model_path for e2e det & rec.")
    ...
    if args.vis_pipeline_save_dir and args.task_type not in (TaskType.DET_REC, TaskType.DET_CLS_REC, TaskType.E2E):
    ...
    need_check_file = {
        "e2e_model_path": args.e2e_model_path,
