...
class PitWrapper(nn.Cell):
    ...
    def _opt_perm_loss(self, pred, target):
        ...
        # loss_mat = self._cal_si_snr(pred, target)
        # # assert (
...
def cal_si_snr(source, estimate_source):
    ...
    #     source_lengths.contiguous().reshape(1, -1, 1).float()
    # )  # [1, B, 1]
    num_samples = ops.Cast()(
        source_lengths.copy().reshape((1, -1, 1)), mstype.float32
    )  # [1, B, 1]
    mean_target = sum(source, 0) / num_samples
    mean_estimate = (
            sum(estimate_source, 0) / num_samples
    )
    zero_mean_target = source - mean_target
    zero_mean_estimate = estimate_source - mean_estimate
    # mask padding position along T
    ...
    s_estimate = zero_mean_estimate  # [T, B, C]
    # s_target = <s', s>s / ||s||^2
    dot = sum(s_estimate * s_target, 0)  # [1, B, C]
    s_target_energy = (sum(s_target ** 2, 0) + EPS)  # [1, B, C]
    proj = dot * s_target / s_target_energy  # [T, B, C]
    # e_noise = s' - s_target
    e_noise = s_estimate - proj  # [T, B, C]
    # SI-SNR = 10 * log_10(||s_target||^2 / ||e_noise||^2)
    sum_ = ops.ReduceSum()
    si_snr_beforelog = sum_(proj ** 2, 0) / (
            sum_(e_noise ** 2, 0) + EPS
    )
    si_snr = 10 * np.log10(si_snr_beforelog + EPS)  # [B, C]
