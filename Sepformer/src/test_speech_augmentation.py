...
class Resample(nn.Cell):
    ...
    def _indices_and_weights(self):
        ...
        # print("max_weight_width = ", max_weight_width)
        ...
        # inside_window_indices = Tensor(self.less(delta_t.abs(), window_width), mstype.int32)
        # print("delta_t.shape = ", delta_t.shape)
        # inside_window_indices = self.less(delta_t.abs(), window_width).astype(mstype.int32)
        inside_window_indices = self.less(delta_t.abs(), window_width)
        # print("inside_window_indices.shape = ", inside_window_indices.shape)
        ...
        # inside_window_indices = index * inside_window_indices
        # print("inside_window_indices.shape = ", inside_window_indices.shape)
        # print("inside_window_indices = ", inside_window_indices)
        # print(" cos = ", self.cos(
        #         2
        #         * math.pi
        #         * lowpass_cutoff
        ...
        for i in range(t_eq_zero_indices.shape[0]):
        ...
        # t_eq_zero_indices = t_eq_zero_indices * index
        ...
        # print("lowpass_cutoff = ", lowpass_cutoff)
...
class SpeedPerturb(nn.Cell):
    ...
    def construct(self, waveform):
        ...
        if self.uniform_real((1, ))[0] > self.perturb_prob:
            return waveform.copy()

        # Perform a random perturbation
        # self.samp_index = torch.randint(len(self.speeds), (1,))[0]
        samp_index = self.uniform_int((1,), Tensor(0, mstype.int32), Tensor(len(self.speeds), mstype.int32))[0]
...
class DropFreq(nn.Cell):
    ...
    def construct(self, waveforms):
        ...
        if self.uniform_real((1,))[0] > self.drop_prob:
